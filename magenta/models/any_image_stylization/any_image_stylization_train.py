...
def main(unused_argv=None):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
      ...
      tf.summary.image('image/0_content_inputs', content_inputs_, 4)
      tf.summary.image('image/1_style_inputs_orig', style_inputs_orig_, 4)
      tf.summary.image('image/2_style_inputs_aug', style_inputs_, 4)
      tf.summary.image('image/3_stylized_images', stylized_images, 4)
